...
"""Extract part of a corpus graph as a standalone graph.
    ...
    uv run python -m src.domain_kg.cli.subset artifacts/graph.nq --list-scopes
    uv run python -m src.domain_kg.cli.subset artifacts/graph.nq --doc thyroid_source --out thyroid.nq
    uv run python -m src.domain_kg.cli.subset artifacts/graph.nq --doc thyroid_source --section "Hormone Synthesis"
    uv run python -m src.domain_kg.cli.subset artifacts/graph.nq --group endocrine --out endocrine.nq
    uv run python -m src.domain_kg.cli.subset artifacts/graph.nq --section s3 --dry-run
...
from .. import native_env as native_env  # noqa: F401 -- pins native thread counts first
from .. import config, console, corpus
...
def main(argv=None) -> int:
    ...
    if args.reason:
        from ..stages import stage7_reason
        report: dict = {}
    ...
    from ..stages import stage8_serve
    stage8_serve.serialize(out_ds, out)
    print()
    print(f"subset -> {out}")
    print(f"query it: python -m src.domain_kg.cli.query {out} relations")
    return 0


def _default_name(args) -> str:
    """A filename that says what the subset is, so two extracts never collide."""
    from ..stages.stage1_parse import slug
