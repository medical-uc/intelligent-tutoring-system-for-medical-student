...
"""Extend the relation ontology — by adoption first, induction second.
    ...
    python -m src.domain_kg.cli.build_ontology --from-srdef  UMLS/META/NET/SRDEF \\
                             --srstre2     UMLS/META/NET/SRSTRE2 \\
                             --out proposals.json

    # 2. Induce: what the corpus expressed that the ontology has no type for.
    python -m src.domain_kg.cli.build_ontology --induce graph.ir.json --llm huatuo --out proposals.json

    # 3. Review proposals.json BY HAND, then merge.
    python -m src.domain_kg.cli.build_ontology --merge proposals.json --into my_ontology.json
...
from .. import native_env as native_env  # noqa: F401 -- thread pinning
from .. import config, console, ontology
...
def main(argv=None) -> int:
    ...
    if args.induce:
        ...
        if args.llm == "claude":
            ...
            call_llm = stage2_extract._call_anthropic
        else:
            call_llm = llm_backends.get_backend(args.llm)[0]
        console.announce_step(
    ...
    print("REVIEW IT, then: python -m src.domain_kg.cli.build_ontology --merge "
