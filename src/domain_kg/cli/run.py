"""Orchestrate the pipeline.
    ...
    uv run python -m src.domain_kg.cli.run --mode <mode> --input ...

Modes:
  stage1       Stage 1 only: the TAGGED markdown that a rewriter produces ->
               a tag-free normalized text file (the provenance anchor every
               chunk offset indexes into) + the Stage-1 IR JSON. Tags map onto
               chunk kinds, so no LLM and no network are involved.
  ...
  stage2       Stage 2 only (NER + LLM modifiers + LLM relations), starting from
               ...
               in isolation. Note that it therefore extracts against RAW NER
               LABELS: a `full` or `corpus` run puts Stage 3 in between the two
               halves of Stage 2 so the extractor works from labels reconciled
               against the linked concept.
  ...
  from-stage3  Stages 4,6,7,8 only, starting from an already-linked IR.
               Needs only rdflib + owlrl -- no models, no network. Use this to
               see the graph-building half run.
  ...
  guards       Re-run ONLY the deterministic relation guards (guards.py) over an
               existing Stage-2 or Stage-3 IR, and write the corrected IR back
               out. No LLM, no models, no network. This is how you audit a graph
               you have already paid to extract: suspect relations are demoted
               to `uncertain` (so Stage 6 reifies them instead of asserting
               them) and each one gets a needs_review entry naming the cue that
               fired. Follow it with `from-stage3` to rebuild the .nq.

The --llm flag (claude | meditron | huatuo) selects the Stage 2 backend (and
Stage 3 rerank) in both `stage2` and `full` modes.

Examples:
  uv run python -m src.domain_kg.cli.run --mode corpus      --input inputs/ --index-dir ./umls_index --llm huatuo
  uv run python -m src.domain_kg.cli.run --mode stage1      --input rewritten.md --out rewritten.stage1.json
  uv run python -m src.domain_kg.cli.run --mode full        --input rewritten.md --index-dir ./umls_index
  uv run python -m src.domain_kg.cli.run --mode stage2      --input sample_stage1_output.json --llm meditron --out stage2_output.json
  uv run python -m src.domain_kg.cli.run --mode from-stage3 --input sample_stage3_output.json
  uv run python -m src.domain_kg.cli.run --mode review      --input graph.ir.json
  uv run python -m src.domain_kg.cli.run --mode guards      --input stage3_output.json --out stage3_guarded.json
  uv run python -m src.domain_kg.cli.run --mode from-stage3 --input stage3_guarded.json --out graph.nq
"""
...
from .. import native_env as native_env  # noqa: E402,F401 -- pins native thread counts before torch/spaCy/faiss
from .. import ontology  # noqa: E402
from ..ir import Document  # noqa: E402
from .. import config  # noqa: E402
from .. import console  # noqa: E402
from ..stages import stage4_postcoord, stage6_assert, stage7_reason, stage8_serve  # noqa: E402
...
def run_stage1(md_path: str, figures_dir="", normalized_out=None,
    ...
    from ..stages import stage1_parse
...
def run_stage2(doc: Document, call_llm=None) -> Document:
    ...
    """
    from ..stages import stage2_extract
...
class LinkerCache:
    ...
    def get(self):
        ...
        if self._linker is None:
            from ..stages import stage3_link
...
def run_stage5(doc: Document, linker, rerank=None) -> Document:
    ...
    from ..stages import stage3_link, stage5_images
...
def run_corpus(inputs, index_dir: str, declared_groups: dict = None,
    ...
    """
    from .. import corpus
    from ..stages import stage6_assert
...
def main():
    ...
    ap.add_argument("--input", required=False, nargs="+",
                    help="tagged .md (stage1/full), or IR JSON. In corpus mode: "
                         "any number of .md files and/or directories to read them from")
    ...
    print(f"domain_kg pipeline  mode {args.mode}")
    ...
    if args.mode == "corpus":
        ...
        print(f"query it : python -m src.domain_kg.cli.query {args.out} relations")
        print(f"subset it: python -m src.domain_kg.cli.subset {args.out} --list-scopes")
        return
    ...
    if is_md and args.mode in ("stage3", "from-stage3", "guards", "review"):
        sys.stderr.write(
            ...
            f"         python -m src.domain_kg.cli.run --mode stage2 --input {args.input} "
            f"--llm {args.llm} --out stage2_output.json\n"
            f"         python -m src.domain_kg.cli.run --mode {args.mode} --input stage2_output.json "
    ...
    if args.mode == "review":
        ...
        print("\n  codes: head-label/tail-label = the span's NER label is not one the "
              ...
              "         (check `--ontology-check`; a label_hierarchy entry may "
    ...
    print("whole run took " + console.format_duration(
        time.time() - run_started_at))
    print(f"query it: python -m src.domain_kg.cli.query {args.out} --list")
